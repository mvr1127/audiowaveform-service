...
app.post("/generate-waveform", async (req, res) => {
  ...
  try {
    ...
    // Check if we have valid samples data
    if (
      !waveform.samples ||
      !Array.isArray(waveform.samples) ||
      waveform.samples.length === 0
    ) {
      console.error("❌ No valid samples data generated by audiowaveform");
      console.error("Waveform structure:", Object.keys(waveform));
      return res.status(500).json({
        error: "Failed to generate valid waveform data",
        details: "audiowaveform did not produce expected samples array",
      });
    }
